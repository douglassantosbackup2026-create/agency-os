      ...
      <aside className="sticky top-0 z-30 hidden h-screen w-60 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:flex">
        <div className="flex items-center gap-2 px-4 py-4">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold">{agency?.name ?? "Retentio"}</div>
            <div className="truncate text-[11px] text-muted-foreground">retentio.app</div>
          </div>
        </div>
        ...
        >
          ...
          <kbd className="hidden rounded bg-surface-2 px-1.5 py-0.5 text-[10px] sm:inline">⌘K</kbd>
        </button>
        ...
        <div className="border-t border-sidebar-border p-3">
          <div className="flex items-center gap-2">
            <div className="grid h-8 w-8 place-items-center rounded-full bg-surface-2 text-xs font-medium">{initials(user?.user_metadata?.display_name || user?.email)}</div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-xs font-medium">{user?.user_metadata?.display_name || user?.email}</div>
              <div className="truncate text-[10px] text-muted-foreground">Owner</div>
            </div>
            <button onClick={signOut} className="rounded p-1.5 text-muted-foreground hover:bg-sidebar-accent hover:text-foreground" title="Sair">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-border bg-background/80 px-5 backdrop-blur">
          <div className="flex items-center gap-3">
            <button onClick={() => setPaletteOpen(true)} className="md:hidden" aria-label="Buscar">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setPaletteOpen(true)} className="hidden items-center gap-1.5 rounded-md border border-border bg-surface px-2.5 py-1.5 text-xs text-muted-foreground hover:bg-surface-2 transition md:inline-flex">
              <Command className="h-3 w-3" /> K
            </button>
            <Link to="/alerts" className="rounded-md p-2 text-muted-foreground hover:bg-surface hover:text-foreground transition">
