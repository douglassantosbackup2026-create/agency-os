...
const DEFAULT_TEMPLATES = [
  {
    name: "Resumo diário",
    type: "daily",
    body: "Olá {{cliente}}! Resumo de hoje: ROAS {{roas}}x, investimento {{spend}}. Tudo certo por aqui.",
  },
  {
    name: "Alerta crítico",
    type: "alert",
    body: "⚠️ {{cliente}}, identificamos uma queda relevante de ROAS. Já estamos investigando.",
  },
  {
    name: "Resumo semanal",
    type: "weekly",
    ...
    >
      <header>
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          Envie alertas, resumos e mensagens diretas via Evolution API.
        </p>
      </header>
          ...
          <div className="space-y-3 p-4">
            ...
            {!import.meta.env.VITE_EVOLUTION_CONFIGURED && (
              <p className="text-[11px] text-amber-500">
                ...
                (status "sent" mas não saem de fato).
              </p>
            )}
          </div>
        </Card>
      ...
      {tab === "templates" && (
        <div className="space-y-4">
          <Card>
            ...
            <div className="space-y-3 p-4">
              <input
                ...
                placeholder="Nome (ex: Alerta crítico)"
          ...
          <Card>
            {data.templates.length === 0 ? (
              <Empty label="Nenhum template ainda." />
            ) : (
                ...
                {data.templates.map((t) => (
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      <button
                        onClick={() => fillTemplateFromSaved(t.body)}
      ...
      {tab === "logs" && (
        <Card>
          {data.logs.length === 0 ? (
            <Empty label="Nenhuma mensagem enviada ainda." />
          ) : (
            <div className="divide-y divide-border">
              {data.logs.map((l: any) => (
                <div key={l.id} className="px-4 py-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span
                        className={`rounded px-1.5 py-0.5 text-[10px] uppercase ${l.status === "sent" ? "bg-emerald-500/10 text-emerald-500" : l.status === "failed" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}
                      >
                        {l.status}
                      </span>
                      <span className="text-xs font-medium">
                        {l.clients?.name ?? l.recipient}
                      </span>
                    </div>
