...
} from "./campaign-audit-helpers.ts";
import {
...
export class CampaignAuditRunnerError extends Error {
  constructor(public status: number, message: string) {
    super(message);
    this.name = "CampaignAuditRunnerError";
  }
}

export type ExecuteCampaignAuditParams = {
...
export async function executeCampaignAudit(p: ExecuteCampaignAuditParams) {
  const { admin, client, client_id, userId, period_days } = p;

  if (await aiBudgetExceeded(admin, client.agency_id as string, 20000)) {
    throw new CampaignAuditRunnerError(
      429,
    ...
    if (Number.isFinite(cooldownMin) && cooldownMin > 0) {
      ...
      if (lastAud?.created_at) {
        ...
        if (elapsedMin < cooldownMin) {
          ...
          throw new CampaignAuditRunnerError(
            429,
            `Auditoria em cooldown. Tente novamente em ~${wait} min.`,
          );
        }
      }
    }
  ...
  "notes": ""
    ...
    try {
      ...
      if (msg === "PAYMENT_REQUIRED") {
        throw new CampaignAuditRunnerError(
          402,
          "Créditos de IA esgotados no gateway.",
        );
      }
      throw e;
    }
