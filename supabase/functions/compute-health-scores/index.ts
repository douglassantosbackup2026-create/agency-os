...
// Public function (no JWT) — protected by service role internally and by /api/public-style cron usage.
...
function computeFor(metrics: M[], lastActivityAgo: number, lastNoteAgo: number) {
  // Split into recent (last 14d) vs prior (15-28d)
  const sorted = [...metrics].sort((a, b) => a.date.localeCompare(b.date));
  const today = sorted.length ? sorted[sorted.length - 1].date : new Date().toISOString().slice(0, 10);
  const cutoff = new Date(new Date(today).getTime() - 14 * 86400000).toISOString().slice(0, 10);
  ...
  const variance = recent.length ? (() => {
    const mean = recRoas;
    const v = recent.reduce((a, b) => a + Math.pow(Number(b.roas ?? 0) - mean, 2), 0) / recent.length;
    return Math.sqrt(v);
  })() : 0;
  ...
  const score = Math.round((performance + optimization + stability + communication + engagement) / 5);
  const risk: "low" | "medium" | "high" = score >= 70 ? "low" : score >= 45 ? "medium" : "high";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const c of clients ?? []) {
      const [{ data: metrics }, { data: lastAct }, { data: lastNote }] = await Promise.all([
        admin.from("metrics_daily").select("date,spend,revenue,roas,cpa,ctr,conversions").eq("client_id", c.id).gte("date", since),
        admin.from("activities").select("created_at").eq("client_id", c.id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
        admin.from("notes").select("created_at").eq("client_id", c.id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      const now = Date.now();
      const lastActAgo = lastAct ? (now - new Date(lastAct.created_at).getTime()) / 86400000 : 14;
      const lastNoteAgo = lastNote ? (now - new Date(lastNote.created_at).getTime()) / 86400000 : 30;
