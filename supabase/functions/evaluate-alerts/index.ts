...
type M = { date: string; spend: number | null; revenue: number | null; roas: number | null; cpa: number | null; ctr: number | null; conversions: number | null };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      try { const b = await req.json(); if (b?.agency_id) agencyFilter = b.agency_id; } catch { /* empty */ }
    }

    let cq = admin.from("clients").select("id, agency_id, name, monthly_budget");
    ...
    for (const c of clients ?? []) {
      const [{ data: metrics }, { data: openAlerts }, { data: lastNote }] = await Promise.all([
        admin.from("metrics_daily").select("date,spend,revenue,roas,cpa,ctr,conversions").eq("client_id", c.id).gte("date", since).order("date"),
        admin.from("alerts").select("type").eq("client_id", c.id).eq("status", "open"),
        admin.from("notes").select("created_at").eq("client_id", c.id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      ...
      const avg = (arr: M[], k: keyof M) => arr.length ? arr.reduce((a, b) => a + Number(b[k] ?? 0), 0) / arr.length : 0;
      const sum = (arr: M[], k: keyof M) => arr.reduce((a, b) => a + Number(b[k] ?? 0), 0);
      ...
      const queue: { type: string; title: string; description: string; priority: "low"|"medium"|"high"|"critical"; recommended_action: string }[] = [];

      if (priRoas > 0 && recRoas < priRoas * 0.8) {
        queue.push({ type: "roas_drop", title: `ROAS caiu ${(((priRoas - recRoas) / priRoas) * 100).toFixed(0)}% — ${c.name}`,
          description: `ROAS recente ${recRoas.toFixed(2)}x vs ${priRoas.toFixed(2)}x anterior.`,
          priority: recRoas < priRoas * 0.6 ? "critical" : "high",
          recommended_action: "Revisar criativos e públicos das campanhas com pior performance." });
      }
      if (priCpa > 0 && recCpa > priCpa * 1.25) {
        queue.push({ type: "cpa_spike", title: `CPA subiu ${(((recCpa - priCpa) / priCpa) * 100).toFixed(0)}% — ${c.name}`,
      ...
      if (priCtr > 0 && recCtr < priCtr * 0.7) {
        queue.push({ type: "ctr_drop", title: `Queda de CTR — ${c.name}`,
      ...
      if (noActivity) {
        queue.push({ type: "campaign_paused", title: `Sem investimento nos últimos 3 dias — ${c.name}`,
          description: "Spend zerado por 3 dias. Verifique se as campanhas pararam.",
          priority: "critical",
          recommended_action: "Verificar status das campanhas e saldo da conta de anúncios." });
      }
      if (c.monthly_budget && totalSpend > Number(c.monthly_budget) * 1.1) {
        queue.push({ type: "budget_pacing", title: `Pacing acima do orçamento — ${c.name}`,
      ...
      const noteAgo = lastNote ? (Date.now() - new Date(lastNote.created_at).getTime()) / 86400000 : 999;
      if (noteAgo > 14) {
        queue.push({ type: "no_contact", title: `Sem contato há ${Math.round(noteAgo)} dias — ${c.name}`,
      ...
      for (const q of queue) {
        ...
        const { error } = await admin.from("alerts").insert({
          ...
          priority: q.priority, status: "open", recommended_action: q.recommended_action,
        });
        if (!error) created++;
      }
    }
