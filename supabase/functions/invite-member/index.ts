...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: caller } = await admin.from("user_roles").select("role").eq("user_id", u.user.id).eq("agency_id", profile.agency_id).maybeSingle();
    ...
    let target = existing.users.find(x => x.email?.toLowerCase() === email.toLowerCase());
    if (!target) {
      const { data: invited, error: inviteErr } = await admin.auth.admin.inviteUserByEmail(email);
      if (inviteErr) return new Response(JSON.stringify({ error: inviteErr.message }), { status: 400, headers: corsHeaders });
      target = invited.user;
    }
    if (!target) return new Response(JSON.stringify({ error: "could not invite" }), { status: 500, headers: corsHeaders });

    // Make sure profile points to this agency (overwrite for invited members)
    await admin.from("profiles").upsert({
      id: target.id, agency_id: profile.agency_id, email: target.email, display_name: target.email?.split("@")[0],
    }, { onConflict: "id" });

    await admin.from("user_roles").upsert({
      user_id: target.id, agency_id: profile.agency_id, role: role ?? "member",
