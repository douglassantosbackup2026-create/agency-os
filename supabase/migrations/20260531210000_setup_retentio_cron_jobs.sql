-- Agendamento idempotente de crons Retentio (chamar uma vez com CRON_SECRET real).
-- SELECT public.setup_retentio_cron_jobs('seu-cron-secret');
...
REVOKE ALL ON FUNCTION public.setup_retentio_cron_jobs(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.setup_retentio_cron_jobs(text) TO service_role;

COMMENT ON FUNCTION public.setup_retentio_cron_jobs(text) IS
  'Agenda crons Retentio (dispatcher, IA, WhatsApp, MV, retenção). Requer CRON_SECRET real.';
